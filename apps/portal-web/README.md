...
Public demo login hands off to `/?role=<tenant|landlord|manager|operator>&email=<email>`. `public-web` uses `NEXT_PUBLIC_PORTAL_WEB_URL` for the portal origin and defaults to `http://127.0.0.1:3001/` during local development. Legacy public `/demo/:role` URLs redirect to this app; role workspace pages must not be implemented in `public-web`.
