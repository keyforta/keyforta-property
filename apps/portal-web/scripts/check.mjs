...
const enLocale = JSON.parse(await readFile(join(root, 'src/locales/en.json'), 'utf8'));
for (const role of ['tenant', 'landlord', 'manager', 'operator']) {
	...
	if (!enLocale.roles?.[role]) throw new Error(`Portal workspace role translations are missing: ${role}`);
}
