...
// Copilot review lesson from PR #137 (Manager phase): the Manager-phase
// lazy chunk/error-boundary wiring had no regression coverage of its own
// at first — only the Landlord path was exercised by
...
// without any test catching it. This task's instructions call out the
// identical risk for the new Tenant wiring
// (`TenantRedesignErrorBoundary`/`TenantRedesignLoading` in
// portal-app.jsx) as a REQUIRED, non-skippable test. Mirrors
// test/redesign/manager-lazy-error-boundary.test.jsx's structure/
// assertions exactly, scoped to the Tenant role and its own legacy
// heading.
vi.mock('@keyforta/ui', () => ({
...
// `React.lazy(...)` for the Tenant redesign in portal-app.jsx rejects
// instead of resolving.
vi.mock('../../src/redesign/tenant/index.jsx', () => {
...
describe('Tenant redesign lazy-load loading state and error fallback', () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  it('renders a visible loading state first, then falls back to the existing legacy tenant shell (not a blank screen) when the lazy chunk import rejects, and logs the failure', async () => {
    ...
    // (b) Once the rejection propagates, the legacy tenant shell (the
