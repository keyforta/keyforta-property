...
// Mirrors test/redesign/kicker-contrast.test.js's coverage for the new
// Manager stylesheet (docs/product/MANAGER_REDESIGN_SPEC.md §2/§3 — the
...
// sizes, aubergine is the approved kicker text color).
...
describe('manager redesign.css kicker labels use a spec-compliant contrast pairing', () => {
  const css = fs.readFileSync(
    path.resolve(__dirname, '../../src/redesign/manager/redesign.css'),
    'utf8',
  );
  ...
  it('does not use --kf-burnished-copper as the .kf-kicker text color', () => {
    const block = ruleBlock('.kf-manager-redesign .kf-kicker');
...
describe('manager redesign.css stays scoped under .kf-manager-redesign', () => {
  const css = fs.readFileSync(
    path.resolve(__dirname, '../../src/redesign/manager/redesign.css'),
    'utf8',
  );
  ...
  it('every non-media/keyframe selector is scoped under .kf-manager-redesign (never bleeds into the unscoped legacy styles.css or .kf-landlord-redesign)', () => {
    const offenders = selectorLines.filter((selector) => (
      !selector.startsWith('@')
      && !selector.startsWith('.kf-manager-redesign')
    ));
