...
| Authenticated role portal | Real sign-in; membership lookup pending | `portal-web` uses a real Microsoft Entra B2B sign-in/sign-out gate (issue #78, PR #103); workspace dashboards show honest empty-state copy pending real read APIs (issue #101). No API endpoint yet resolves a signed-in identity to its organization/role, so authenticated users land on "workspace access pending" until that lookup ships — blocked on a migration-sequencing decision (see `apps/portal-web/README.md`) |
| Public rental application prototype | Non-authoritative mock; scope now approved (PRD amended, issue #75) | Legacy public-web route stores browser-local data only; needs a real authenticated-to-API rebuild (persistence, validation, human-review gate) — no longer a scope conflict, but not yet backed by a durable API |
